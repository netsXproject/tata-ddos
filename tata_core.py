...
class TATAEngine:
    """Main attack engine with proxy support"""
    ...
    def generate_report(self, filename: str) -> str:
        ...
        try:
            ...
            c.drawString(100, 730, f"Target: {self.target}:{self.port}")
            c.drawString(100, 710, f"Duration: {self.duration}s")
            c.drawString(100, 690, f"Total Packets: {self.stats['packets_sent']:,}")
            c.drawString(100, 670, f"Errors: {self.stats['errors']}")
            c.drawString(100, 650, f"Peak PPS: {max(self.stats['peaks']):,}" if self.stats['peaks'] else "N/A")
            
            if self.proxy_manager:
                proxy_stats = self.proxy_manager.get_stats()
                c.drawString(100, 610, "Proxy Statistics:")
                c.drawString(120, 590, f"Total Proxies: {proxy_stats['total_proxies']}")
                c.drawString(120, 570, f"Blacklisted: {proxy_stats['blacklisted']}")
                c.drawString(120, 550, f"Used: {proxy_stats['used_count']}")
                c.drawString(120, 530, f"Avg Latency: {proxy_stats['avg_latency']:.2f}ms")
